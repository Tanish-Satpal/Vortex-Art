...
# Event Horizon
EVENT_HORIZON_RADIUS = 0.3  # Radius of the pure black center (0-1 scale)
GLOW_FALLOFF = 3.0  # How quickly brightness falls off from edge (higher = faster)
...
FILAMENT_PHASE_SHIFT = 1.5  # Phase shift for variety
...
# Output
OUTPUT_FILENAME = 'blackhole_vortex_7.png'
DPI = 150
...
def create_blackhole_vortex():
    ...
    # Create aggressive brightness falloff from event horizon
    # Very bright right at the edge, fading quickly outward
    distance_from_horizon = R - EVENT_HORIZON_RADIUS

    # Glow function: exponential falloff
    glow = np.exp(-GLOW_FALLOFF * np.maximum(distance_from_horizon, 0))

    # Inner rim super-glow
    inner_rim = np.exp(-50 * (distance_from_horizon - 0.05) ** 2) * INNER_GLOW_INTENSITY

    # Combine filaments with glow
    intensity = intensity * glow + inner_rim

    # Apply event horizon mask (pure black center)
    horizon_mask = (R > EVENT_HORIZON_RADIUS).astype(float)
    intensity = intensity * horizon_mask
